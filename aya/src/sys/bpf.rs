...
pub(crate) fn bpf_pin_object(fd: BorrowedFd<'_>, path: &CStr) -> io::Result<()> {
    ...
    sys_bpf_unit(bpf_cmd::BPF_OBJ_PIN, &mut attr)
}
...
fn lookup<K: Pod, V: Pod>(
    ...
    match sys_bpf_unit(cmd, &mut attr) {
...
pub(crate) fn bpf_map_lookup_elem_ptr<K: Pod, V>(
    ...
    match sys_bpf_unit(bpf_cmd::BPF_MAP_LOOKUP_ELEM, &mut attr) {
        Ok(_) => Ok(Some(())),
        Err(io_error) if io_error.raw_os_error() == Some(ENOENT) => Ok(None),
        Err(e) => Err(e),
    }
...
pub(crate) fn bpf_map_update_elem<K: Pod, V: Pod>(
    ...
    sys_bpf_unit(bpf_cmd::BPF_MAP_UPDATE_ELEM, &mut attr)
}
...
pub(crate) fn bpf_map_delete_elem<K: Pod>(fd: BorrowedFd<'_>, key: &K) -> io::Result<()> {
    ...
    sys_bpf_unit(bpf_cmd::BPF_MAP_DELETE_ELEM, &mut attr)
}
...
pub(crate) fn bpf_map_get_next_key<K: Pod>(
    ...
    match sys_bpf_unit(bpf_cmd::BPF_MAP_GET_NEXT_KEY, &mut attr) {
...
pub(crate) fn bpf_map_freeze(fd: BorrowedFd<'_>) -> io::Result<()> {
    ...
    let u = unsafe { &mut attr.__bindgen_anon_2 };
    u.map_fd = fd.as_raw_fd() as u32;
    sys_bpf_unit(bpf_cmd::BPF_MAP_FREEZE, &mut attr)
}
...
pub(crate) fn bpf_link_update(
    ...
    sys_bpf_unit(bpf_cmd::BPF_LINK_UPDATE, &mut attr)
}
...
pub(crate) fn bpf_prog_attach(
    ...
    sys_bpf_unit(bpf_cmd::BPF_PROG_ATTACH, &mut attr).map_err(|io_error| SyscallError {
...
pub(crate) fn bpf_prog_detach(
    ...
    sys_bpf_unit(bpf_cmd::BPF_PROG_DETACH, &mut attr).map_err(|io_error| SyscallError {
...
pub(crate) fn bpf_prog_query(
    ...
    let ret = sys_bpf_unit(bpf_cmd::BPF_PROG_QUERY, &mut attr);
...
fn bpf_obj_get_info_by_fd<T, F: FnOnce(&mut T)>(
    ...
    match sys_bpf_unit(bpf_cmd::BPF_OBJ_GET_INFO_BY_FD, &mut attr) {
        Ok(()) => Ok(info),
...
fn sys_bpf_unit(cmd: bpf_cmd, attr: &mut bpf_attr) -> io::Result<()> {
